...
from .constraints import FixedConstraintModel, FrameConstraintModel, FrameSpec, LinearConstraintModel
...
class Layout:
    ...
    def _constrain_frame_geometry(self) -> None:
            ...
            for j in range(i + 1, len(allocations)):
                ...
                if min(first.bottom, second.bottom) > max(first.top, second.top):
                    ...
                    if before >= 0:
                        ...
                        minimum_gap = max(minimum_gap, margins)
                        preferred_gap = max(preferred_gap, margins + residual)
        ...
        if not horizontal_neighbors and not vertical_neighbors:
            return
        frames, _ = self._frame_constraint_model(specs, horizontal_neighbors, vertical_neighbors, minimum_gap, preferred_gap)
